<?php			
		...
		try
			...
			if(isset($_POST['guncelle']))
			{
				...
				{
					header('Location:../EtkinlikGuncelle.php?guncelle=no&id='.$id);
				}
				else
				{			
					$sorgu = mysqli_query($db, "UPDATE etkinlik SET Baslik='".$baslik."', Tarih='".$tarih."', Aciklama='".$aciklama."', DurumAnasayfa=".$AnaAktif.", DurumTumEtkinlik=".$TumAktif." WHERE EtkinlikID=".$id);
					
					if($_POST['DosyaSilOnay']==on)
					{
						//Dosyaları Sil
						$sor = mysqli_query($db,"select * from etkinlikdosya where EtkinlikID=".$id);
						...
						if($dosya_sayi>0)
						{
							...
							{ 
								...
								{ 
									...
									$yol="Dosyalar/Etkinlik";		
									$uploadimage = mysqli_query($db,"INSERT INTO etkinlikdosya (EtkinlikID,DosyaYol) VALUES (".$id.",'".$yol."/".$yeniDosyaAdi."')");
								} 
							}
						}
					...
					if($sorgu)
					{
						header('Location:../Etkinlikler.php?guncellendi=yes');
					}
					if(!$sorgu)
					{
						header('Location:../EtkinlikGuncelle.php?guncelle=no&id='.$id);

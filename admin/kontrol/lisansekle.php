<?php			
		...
		try
			...
			if(isset($_POST['ekle']))
			{
				...
				{
					header('Location:../LisansEkle.php?ekle=no');
				}
				else
				{	
					$yol;
					...
					$sira=mysqli_num_rows(mysqli_query($db,"SELECT Sira FROM lisans ORDER BY Sira"))+1;
					$sorgu = mysqli_query($db,"INSERT INTO lisans (Baslik, Link, Durum, Sira) VALUES ('".$baslik."','".$yol.$yeniDosyaAdi."',".$AnaAktif.",".$sira.")");			

					
					if($sorgu)
					{
						header('Location:../Lisans.php?eklendi=yes');
					}
					if(!$sorgu)
					{
						header('Location:../LisansEkle.php?ekle=no');

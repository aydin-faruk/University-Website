<?php			
		...
		try
			...
			if(isset($_POST['ekle']))
			{
				...
				{
					header('Location:../EtkinlikEkle.php?ekle=no');
				}
				else
				{	
					$sorgu = mysqli_query($db,"INSERT INTO etkinlik (Baslik,Aciklama,Tarih,DurumAnasayfa,DurumTumEtkinlik) VALUES ('".$baslik."','".$aciklama."','".$tarih."',".$AnaAktif.",".$TumAktif.")");
					$idAl=mysqli_query($db,"SELECT * FROM etkinlik ORDER BY EtkinlikID DESC LIMIT 1");
					...
					$dosya_sayi=count($_FILES['Dosya']['name']); 
						...
						{ 
							...
							{	
									...
									$uploadimage = mysqli_query($db,"INSERT INTO etkinlikdosya (EtkinlikID,DosyaYol) VALUES (".$sid['EtkinlikID'].",'".$yol."/".$yeniDosyaAdi."')");
							} 
						}
					...
					if($sorgu)
					{
						header('Location:../Etkinlikler.php?eklendi=yes');
					}
					if(!$sorgu)
					{
						header('Location:../EtkinlikEkle.php?ekle=no');

<?php			
		...
		try
			...
			if(isset($_POST['ekle']))
			{
				...
				{
					header('Location:../LisansUstuEkle.php?ekle=no');
				}
				else
				{	
					$yol;
					...
					$sira=mysqli_num_rows(mysqli_query($db,"SELECT Sira FROM lisansustu ORDER BY Sira"))+1;
					$sorgu = mysqli_query($db,"INSERT INTO lisansustu (Baslik, Link, Durum, Sira) VALUES ('".$baslik."','".$yol.$yeniDosyaAdi."',".$AnaAktif.",".$sira.")");			

					
					if($sorgu)
					{
						header('Location:../LisansUstu.php?eklendi=yes');
					}
					if(!$sorgu)
					{
						header('Location:../LisansUstuEkle.php?ekle=no');

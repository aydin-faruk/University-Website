<?php			
		...
		try
			...
			if(isset($_POST['guncelle']))
			{
				...
				{
					header('Location:../LisansGuncelle.php?guncelle=no&id='.$id);
				}
				else
				{			
					if(!empty($_FILES['Dosya']['name'][$i]))
					{
						...
						//Dosyaları Sil
						$sor = mysqli_query($db,"select * from lisans where LisansID=".$id);
						...
						$sorgu = mysqli_query($db, "UPDATE lisans SET Baslik='".$baslik."', Link='".$yol.$yeniDosyaAdi."',Durum=".$AnaAktif." WHERE LisansID=".$id);
					}
					else
					{
						$sorgu = mysqli_query($db, "UPDATE lisans SET Baslik='".$baslik."', Durum=".$AnaAktif." WHERE LisansID=".$id);
					}	
					
					if($sorgu)
					{
						header('Location:../Lisans.php?guncellendi=yes');
					}
					if(!$sorgu)
					{
						header('Location:../LisansGuncelle.php?guncelle=no&id='.$id);

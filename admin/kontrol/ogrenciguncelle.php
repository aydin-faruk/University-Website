<?php			
		...
		try
			...
			if(isset($_POST['guncelle']))
			{
				...
				{
					header('Location:../OgrenciGuncelle.php?guncelle=no&id='.$id);
				}
				else
				{			
					...
					{
						//Dosyaları Sil
						$sor = mysqli_query($db,"select * from ogrenci where OgrenciID=".$id);
						...
						$sorgu = mysqli_query($db, "UPDATE ogrenci SET Baslik='".$baslik."', Link='".$yol.$yeniDosyaAdi."',Durum=".$AnaAktif." WHERE OgrenciID=".$id);
					}
					else
					{
						$sorgu = mysqli_query($db, "UPDATE ogrenci SET Baslik='".$baslik."',Durum=".$AnaAktif." WHERE OgrenciID=".$id);
					}	
					
					if($sorgu)
					{
						header('Location:../Ogrenci.php?guncellendi=yes');
					}
					if(!$sorgu)
					{
						header('Location:../OgrenciGuncelle.php?guncelle=no&id='.$id);

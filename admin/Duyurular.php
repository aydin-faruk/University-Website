...
<section class="panel">
    <header class="panel-heading">
        ...
        <h2 class="panel-title">Genel Duyuru İşlemleri</h2>
    </header>
        ...
        <table class="table table-bordered table-striped mb-none" id="datatable-editable">
            <thead>
                <tr>
                    ...
                    <th>Yayın Sonrası Silinecek mi?</th>
            ...
            <tbody>
					<?php
						...
						if($_SESSION['yetki']==1)
						{
							$sorgu = mysqli_query($db,"SELECT * FROM duyuru d, akademikpersonel ap WHERE d.APersonelID=ap.APersonelID ORDER BY d.Tarih DESC");
						}
						else if($_SESSION['yetki']==2)
						{
							$sorgu = mysqli_query($db,"SELECT * FROM duyuru d, akademikpersonel ap WHERE d.APersonelID=ap.APersonelID AND ap.APersonelID=".mysqli_real_escape_string($db,$_SESSION['id'])." ORDER BY d.Tarih DESC");
						}
                    ...
                    <tr class="gradeX">
						...
						?>
                        <td><?php echo date("d.m.Y", strtotime($satir["SonTarih"])); ?></td>
                        <td>
							<?php 
								if($satir["DurumAnasayfa"]==1) echo "Evet";
								if($satir["DurumAnasayfa"]==0) echo "Hayır"; 		
							?>
						</td>
                        <td>
							<?php 
                        ...
                        <td class="actions">
                            <a href="DuyuruGuncelle.php?id=<?php echo $satir['DuyuruID']; ?>"><i style="font-size:25px; color:#016d2f;" class="fa fa-pencil"></i></a>                          
                            <a onclick="gonder(<?php echo $satir['DuyuruID']; ?>)" data-toggle="modal" style="cursor:pointer;" class="on-default remove-row"><i style="font-size:25px; color:red;" class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
						<?php			
